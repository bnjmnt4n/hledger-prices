...
func main() {
	...
	if currencies_str != "" {
		...
		currencies_chan := make(chan currencyExchangeRate)
		currency_symbols := strings.Split(currencies_str, ",")
		num_errors += len(currency_symbols)
		go getCurrencies(currency_symbols, currencies_chan, errors_chan)

		for exchange_rate := range currencies_chan {
	...
	if stock_symbols_str != "" {
		...
		stocks_chan := make(chan stock)
		stock_symbols := strings.Split(stock_symbols_str, ",")
		num_errors += len(stock_symbols)
		go getStockPrices(stock_symbols, stocks_chan, errors_chan)

		for stock := range stocks_chan {
			_, err := fmt.Printf("P %s %s USD %f", stock.Date.Format("2006/01/02"), stock.Symbol, stock.Price)
			fmt.Println("")
			if err != nil {
				// TODO: handle errors.
			}
